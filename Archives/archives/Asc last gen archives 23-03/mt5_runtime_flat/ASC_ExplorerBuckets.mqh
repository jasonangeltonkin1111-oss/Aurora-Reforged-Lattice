...
struct ASC_PreparedBucketState
  {
   ...
   ASC_SubBucketViewModel       sub_buckets[];
   int                          batch_ready[];
   int                          batch_pending[];
   int                          batch_reused[];
   ...
   for(int i=0;i<6;i++)
     {
      ...
      prepared.buckets[i].note=(i==5
                                ? "Top-level stock market map. Regional stock groups appear first, then finer stock detail arrives in the background."
                                : "Top-level market map built from current classified symbol coverage.");
...
string ASC_CL_MainBucketNote(const ASC_SymbolClassification &classification)
  {
   string note="Top-level market map built from current classified symbol coverage.";
   if(classification.primary_bucket!="")
      note+=" Working group " + classification.primary_bucket + ".";
   if(classification.asset_class=="STOCK")
     {
      string secondary_group=ASC_CL_StockSecondaryGroupName(classification);
      if(secondary_group!="")
         note+=" Regional stock set " + secondary_group + ".";
      if(classification.theme_bucket!="")
         note+=" Region/theme " + classification.theme_bucket + ".";
...
string ASC_SubBucketNormalizeSourceKey(const string raw_value)
  {
   string value=ASC_ToUpper(ASC_Trim(raw_value));
   if(value=="")
      value="OTHER";
   return(value);
  }

string ASC_SubBucketKey(const string main_bucket_id,const string source_key)
  {
   return(ASC_ToLower(ASC_Trim(main_bucket_id)) + "|" + ASC_SubBucketNormalizeSourceKey(source_key));
  }

string ASC_SubBucketHumanNameFromSource(const string source_value)
  {
   string value=ASC_Trim(source_value);
   if(value=="")
      return("Other");
   return(ASC_TitleCase(value));
  }

string ASC_PreparedSymbolSubBucketSourceKey(const ASC_BucketPreparedSymbol &symbol)
  {
   string main_bucket=ASC_ToLower(ASC_Trim(symbol.bucket_id));
   if(main_bucket=="stocks")
     {
      if(ASC_Trim(symbol.secondary_group_id)!="")
         return(ASC_SubBucketNormalizeSourceKey(symbol.secondary_group_id));
      if(ASC_Trim(symbol.theme_bucket)!="")
         return(ASC_SubBucketNormalizeSourceKey(symbol.theme_bucket));
      return("OTHER");
     }
   if(ASC_Trim(symbol.primary_bucket)!="")
      return(ASC_SubBucketNormalizeSourceKey(symbol.primary_bucket));
   return("OTHER");
  }

string ASC_PreparedSymbolSubBucketName(const ASC_BucketPreparedSymbol &symbol)
  {
   string main_bucket=ASC_ToLower(ASC_Trim(symbol.bucket_id));
   if(main_bucket=="stocks")
     {
      if(ASC_Trim(symbol.secondary_group_name)!="")
         return(symbol.secondary_group_name);
      if(ASC_Trim(symbol.theme_bucket)!="")
         return(ASC_SubBucketHumanNameFromSource(symbol.theme_bucket));
      return("Other");
     }
   if(ASC_Trim(symbol.primary_bucket)!="")
     {
      if(main_bucket=="fx")
         return("FX " + ASC_SubBucketHumanNameFromSource(StringSubstr(symbol.primary_bucket,3)));
      if(main_bucket=="indices")
         return("Index " + ASC_SubBucketHumanNameFromSource(StringSubstr(symbol.primary_bucket,6)));
      if(main_bucket=="crypto")
         return("Crypto " + ASC_SubBucketHumanNameFromSource(StringSubstr(symbol.primary_bucket,7)));
      if(main_bucket=="metals")
         return("Metals " + ASC_SubBucketHumanNameFromSource(StringSubstr(symbol.primary_bucket,7)));
      return(ASC_SubBucketHumanNameFromSource(symbol.primary_bucket));
     }
   return("Other");
  }

string ASC_PreparedSymbolSubBucketNote(const ASC_BucketPreparedSymbol &symbol)
  {
   string main_bucket=ASC_ToLower(ASC_Trim(symbol.bucket_id));
   if(main_bucket=="stocks")
     {
      if(ASC_Trim(symbol.theme_bucket)!="")
         return("Stock working set grouped by region or theme for operator navigation.");
      return("Fallback stock working set for names that do not carry richer grouping yet.");
     }

   if(ASC_Trim(symbol.sector)!="")
      return("Current market group with added sector context.");
   if(ASC_Trim(symbol.theme_bucket)!="")
      return("Current market group with added theme context.");
   return("Current market group from prepared symbol coverage.");
  }

int ASC_SubBucketFindByKey(const ASC_SubBucketViewModel &sub_buckets[],const string key)
  {
...
void ASC_SubBucketSort(ASC_SubBucketViewModel &views[])
  {
   int total=ArraySize(views);
   for(int i=1;i<total;i++)
     {
      ASC_SubBucketViewModel key=views[i];
      int j=i-1;
      while(j>=0)
        {
         bool keep=false;
         if(views[j].open_symbol_count!=key.open_symbol_count)
            keep=(views[j].open_symbol_count>key.open_symbol_count);
         else if(views[j].total_symbol_count!=key.total_symbol_count)
            keep=(views[j].total_symbol_count>key.total_symbol_count);
         else
            keep=(StringCompare(views[j].name,key.name,true)<=0);
         if(keep)
            break;
         views[j+1]=views[j];
         j--;
...
void ASC_PreparedRebuildSubBuckets(ASC_PreparedBucketState &prepared)
  {
   ArrayResize(prepared.sub_buckets,0);
   for(int i=0;i<ArraySize(prepared.symbols);i++)
     {
      ASC_BucketPreparedSymbol symbol=prepared.symbols[i];
      string main_bucket_id=ASC_ToLower(ASC_Trim(symbol.bucket_id));
      if(main_bucket_id=="")
         continue;
      string source_key=ASC_PreparedSymbolSubBucketSourceKey(symbol);
      string key=ASC_SubBucketKey(main_bucket_id,source_key);
      int slot=ASC_SubBucketFindByKey(prepared.sub_buckets,key);
      if(slot<0)
        {
         slot=ArraySize(prepared.sub_buckets);
         ArrayResize(prepared.sub_buckets,slot+1);
         prepared.sub_buckets[slot].main_bucket_id=main_bucket_id;
         prepared.sub_buckets[slot].key=key;
         prepared.sub_buckets[slot].source_key=source_key;
         prepared.sub_buckets[slot].name=ASC_PreparedSymbolSubBucketName(symbol);
         prepared.sub_buckets[slot].note=ASC_PreparedSymbolSubBucketNote(symbol);
         prepared.sub_buckets[slot].total_symbol_count=0;
         prepared.sub_buckets[slot].open_symbol_count=0;
         ArrayResize(prepared.sub_buckets[slot].symbol_indices,0);
        }
      int symbol_slot=ArraySize(prepared.sub_buckets[slot].symbol_indices);
      ArrayResize(prepared.sub_buckets[slot].symbol_indices,symbol_slot+1);
      prepared.sub_buckets[slot].symbol_indices[symbol_slot]=i;
      prepared.sub_buckets[slot].total_symbol_count++;
      if(symbol.open_now)
         prepared.sub_buckets[slot].open_symbol_count++;
     }
   ASC_SubBucketSort(prepared.sub_buckets);
  }

int ASC_PreparedFilteredSubBuckets(const ASC_PreparedBucketState &prepared,const string main_bucket_id,const ASC_ExplorerMarketFilter filter,ASC_SubBucketViewModel &filtered[])
  {
   ArrayResize(filtered,0);
   string wanted_main=ASC_ToLower(ASC_Trim(main_bucket_id));
   for(int i=0;i<ArraySize(prepared.sub_buckets);i++)
     {
      if(prepared.sub_buckets[i].main_bucket_id!=wanted_main)
         continue;
      if(filter==ASC_EXPLORER_FILTER_OPEN_ONLY && prepared.sub_buckets[i].open_symbol_count<=0)
         continue;
      if(prepared.sub_buckets[i].total_symbol_count<=0)
         continue;
      int slot=ArraySize(filtered);
      ArrayResize(filtered,slot+1);
      filtered[slot]=prepared.sub_buckets[i];
     }
   return(ArraySize(filtered));
  }

int ASC_PreparedSubBucketVisibleSymbolCount(const ASC_SubBucketViewModel &sub_bucket,const ASC_ExplorerMarketFilter filter)
  {
   if(filter==ASC_EXPLORER_FILTER_OPEN_ONLY)
      return(sub_bucket.open_symbol_count);
   return(sub_bucket.total_symbol_count);
  }

int ASC_PreparedSubBucketSymbols(const ASC_PreparedBucketState &prepared,const ASC_SubBucketViewModel &sub_bucket,const ASC_ExplorerMarketFilter filter,ASC_BucketPreparedSymbol &resolved[])
  {
   ArrayResize(resolved,0);
   for(int i=0;i<ArraySize(sub_bucket.symbol_indices);i++)
     {
      int symbol_index=sub_bucket.symbol_indices[i];
      if(symbol_index<0 || symbol_index>=ArraySize(prepared.symbols))
         continue;
      ASC_BucketPreparedSymbol symbol=prepared.symbols[symbol_index];
      if(filter==ASC_EXPLORER_FILTER_OPEN_ONLY && !symbol.open_now)
         continue;
      int slot=ArraySize(resolved);
      ArrayResize(resolved,slot+1);
      resolved[slot]=symbol;
     }
...
void ASC_CopyPreparedBucketState(const ASC_PreparedBucketState &source,ASC_PreparedBucketState &target)
  {
   ...
   ArrayResize(target.sub_buckets,ArraySize(source.sub_buckets));
   for(int i=0;i<ArraySize(source.sub_buckets);i++)
     {
      target.sub_buckets[i].main_bucket_id=source.sub_buckets[i].main_bucket_id;
      target.sub_buckets[i].key=source.sub_buckets[i].key;
      target.sub_buckets[i].source_key=source.sub_buckets[i].source_key;
      target.sub_buckets[i].name=source.sub_buckets[i].name;
      target.sub_buckets[i].note=source.sub_buckets[i].note;
      target.sub_buckets[i].total_symbol_count=source.sub_buckets[i].total_symbol_count;
      target.sub_buckets[i].open_symbol_count=source.sub_buckets[i].open_symbol_count;
      ArrayResize(target.sub_buckets[i].symbol_indices,ArraySize(source.sub_buckets[i].symbol_indices));
      for(int j=0;j<ArraySize(source.sub_buckets[i].symbol_indices);j++)
         target.sub_buckets[i].symbol_indices[j]=source.sub_buckets[i].symbol_indices[j];
     }
...
int ASC_PreparedFindSymbolStateIndex(const ASC_BucketPreparedSymbol &symbol,ASC_SymbolState &states[],const int count)
  {
   if(symbol.symbol_state_index>=0 && symbol.symbol_state_index<count)
     {
      if(states[symbol.symbol_state_index].symbol==symbol.live_symbol
         || ASC_SymbolsMatchCanonical(states[symbol.symbol_state_index].symbol,symbol.live_symbol))
         return(symbol.symbol_state_index);
     }

   for(int i=0;i<count;i++)
     {
      if(states[i].symbol==symbol.live_symbol || ASC_SymbolsMatchCanonical(states[i].symbol,symbol.live_symbol))
         return(i);
     }
   return(-1);
  }

void ASC_PreparedRefreshLiveState(ASC_PreparedBucketState &prepared,ASC_SymbolState &states[],const int count)
  {
   if(!prepared.ready)
      return;
   ...
   for(int i=0;i<ArraySize(prepared.symbols);i++)
     {
      int state_index=ASC_PreparedFindSymbolStateIndex(prepared.symbols[i],states,count);
      ASC_MarketStatus market_status=ASC_MARKET_UNKNOWN;
      bool open_now=false;
      if(state_index>=0)
        {
         market_status=states[state_index].market_status;
         open_now=(states[state_index].market_status==ASC_MARKET_OPEN);
        }

      if(prepared.symbols[i].symbol_state_index!=state_index
         || prepared.symbols[i].market_status!=market_status
         || prepared.symbols[i].open_now!=open_now)
        {
         prepared.symbols[i].symbol_state_index=state_index;
         prepared.symbols[i].market_status=market_status;
         prepared.symbols[i].open_now=open_now;
         changed=true;
        }
   ...
   ASC_PreparedRebuildBucketsFromSymbols(prepared);
   ASC_PreparedPhaseBucketSort(prepared);
   ASC_PreparedPhaseSymbolReorder(prepared);
   ASC_PreparedRefreshBucketPercentages(prepared);
   ASC_PreparedRebuildSubBuckets(prepared);
  }

bool ASC_PreparedSymbolFilterEligible(const ASC_BucketPreparedSymbol &symbol,ASC_SymbolState &states[],const int count)
  {
   int state_index=ASC_PreparedFindSymbolStateIndex(symbol,states,count);
   if(state_index<0)
      return(false);
   return(states[state_index].filter_readiness==ASC_FILTER_ELIGIBLE && states[state_index].filter_eligible);
  }

int ASC_PreparedBucketEligibleCount(const ASC_PreparedBucketState &prepared,const ASC_BucketViewModel &bucket,ASC_SymbolState &states[],const int count)
  {
   int eligible_count=0;
   ASC_BucketPreparedSymbol resolved[];
   int total=ASC_PreparedBucketSymbols(prepared,bucket,resolved);
   for(int i=0;i<total;i++)
     {
      if(ASC_PreparedSymbolFilterEligible(resolved[i],states,count))
         eligible_count++;
     }
   return(eligible_count);
  }

int ASC_PreparedSubBucketEligibleCount(const ASC_PreparedBucketState &prepared,const ASC_SubBucketViewModel &sub_bucket,ASC_SymbolState &states[],const int count)
  {
   int eligible_count=0;
   for(int i=0;i<ArraySize(sub_bucket.symbol_indices);i++)
     {
      ...
      if(symbol_index<0 || symbol_index>=ArraySize(prepared.symbols))
         continue;
      if(ASC_PreparedSymbolFilterEligible(prepared.symbols[symbol_index],states,count))
         eligible_count++;
     }
   return(eligible_count);
  }
