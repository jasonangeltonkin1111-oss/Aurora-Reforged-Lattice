#property strict
#property version   "1.735"
#property description "ISSX single-wrapper consolidated kernel (safe attach wrapper)"
#define ISSX_PATCH_SIGNATURE                         "attach_recovery_contract_split_v2"
...
string              g_last_ea1_root_payload_state    = "unknown";
string              g_last_ea1_root_projection_state = "unknown";
...
string              g_market_status_json_file_name = "";
string              g_market_log_file_name         = "";
string              g_market_json_relative_path    = "";
string              g_market_status_json_relative_path = "";
...
string ISSX_BuildSignature()
  {
   return "engine="+ISSX_ENGINE_VERSION+
          " patch="+ISSX_PATCH_SIGNATURE+
          " build="+IntegerToString((int)__MQLBUILD__)+
          " source="+__FILE__;
  }

void ISSX_LogStageProjectResult(const string stage_tag,
                                const bool persist_ok,
                                const bool debug_ok,
                                const bool root_ok,
                                const string extra_reason)
  {
   g_debug.Write(((persist_ok && debug_ok && root_ok)?"INFO":"ERROR"),stage_tag,
                 ((persist_ok && debug_ok && root_ok)?"project_ok":"project_fail"),
                 "persist="+ISSX_OnOff(persist_ok)+
                 " debug="+ISSX_OnOff(debug_ok)+
                 " root="+ISSX_OnOff(root_ok)+
                 " reason="+extra_reason);
  }
...
void ISSX_ResolveOperatorContext()
  {
   ...
   g_market_status_json_file_name="market_status_"+g_operator_server_name_safe+"_"+login_safe+ISSX_JSON_EXT;
   ...
   g_market_status_json_relative_path=ISSX_Util::JoinPath(g_operator_root_relative,g_market_status_json_file_name);
...
bool ISSX_ProjectEA1(const string stage_json,
                     ...
                     string &out_reason)
  {
   ...
   g_debug.Write("INFO","ea1_publish","publish_prepare_operator_paths_start",
                 "checkpoint=publish_prepare_operator_paths_start root="+g_operator_root_relative+" payload_json="+g_market_json_relative_path+" status_json="+g_market_status_json_relative_path+" log="+g_market_log_relative_path);
   const bool operator_paths_ok=(StringLen(g_market_json_relative_path)>0 && StringLen(g_market_status_json_relative_path)>0 && StringLen(g_market_log_relative_path)>0 && StringLen(g_market_rolling_json_relative_path)>0);
   g_debug.Write((operator_paths_ok?"INFO":"ERROR"),"ea1_publish",(operator_paths_ok?"publish_prepare_operator_paths_ok":"publish_prepare_operator_paths_fail"),
                 "payload_json_path="+g_market_json_relative_path+" status_json_path="+g_market_status_json_relative_path+" log_path="+g_market_log_relative_path+" reason="+(operator_paths_ok?"ok":"empty_operator_path"));
...
if(StringLen(broker_dump_json)>2)
  {
   ...
   fs_market.Reset();
   g_debug.Write("INFO","ea1_publish","publish_root_status_write_start","checkpoint=publish_root_payload_write_start final_path="+g_market_json_relative_path+" bytes="+IntegerToString(ISSX_DataHandler::EstimateUtf8Bytes(stage_json)));
   const bool operator_json_ok=ISSX_DataHandler::WritePayloadAtomic(g_market_json_relative_path,stage_json,fs_market,true);
   g_last_ea1_root_payload_state=(operator_json_ok?"success":"failed");
   g_debug.Write((operator_json_ok?"INFO":"ERROR"),"ea1_publish",(operator_json_ok?"publish_root_payload_write_success":"publish_root_payload_write_fail"),
   ...
   g_debug.Write((operator_json_ok?"INFO":"ERROR"),"ea1_publish",(operator_json_ok?"json_write_success":"json_write_fail"),
                 "path="+g_market_json_relative_path+" bytes="+IntegerToString(fs_market.payload_bytes_written)+" reason="+(operator_json_ok?"ok":"root_payload_projection_failed"));
   ...
   g_debug.Write("INFO","ea1_publish","publish_root_projection_start","payload_json="+g_market_json_relative_path+" status_json="+g_market_status_json_relative_path+" log="+g_market_log_relative_path);
   ...
   g_last_ea1_root_projection_state=(root_projection_ok?"success":"failed");
   g_debug.Write((root_projection_ok?"INFO":"ERROR"),"ea1_publish",(root_projection_ok?"publish_root_projection_ok":"publish_root_projection_fail"),
                 "payload_json="+g_last_ea1_root_payload_state+" status_json="+g_last_ea1_root_status_state+" log="+g_last_ea1_root_debug_state);
   ...
   const bool core_publish_ok=(stage_write_ok && debug_write_ok && universe_ok);
   const bool publish_ok=core_publish_ok;
   g_ea1.publish_payload_bytes_written=(core_publish_ok ? (g_ea1.publish_stage_json_bytes+g_ea1.publish_debug_json_bytes+g_ea1.publish_universe_json_bytes) : 0);
   g_last_ea1_publish_state=(core_publish_ok ? (root_projection_ok?"success":"degraded") : "failed");
   g_last_ea1_publish_reason=(core_publish_ok?"ok":"stage="+g_last_ea1_stage_write_state+" debug="+g_last_ea1_debug_write_state+" universe="+g_last_ea1_universe_write_state);
   if(core_publish_ok && !root_projection_ok)
      g_last_ea1_publish_reason="core_publish_ok root_projection="+g_last_ea1_root_projection_state+" status_json="+g_last_ea1_root_status_state+" payload_json="+g_last_ea1_root_payload_state+" root_log="+g_last_ea1_root_debug_state;
   out_reason=g_last_ea1_publish_reason;

   g_debug.Write((core_publish_ok?"INFO":"ERROR"),"ea1_publish",(core_publish_ok?"publish_complete":"publish_failed"),
                 "reason="+out_reason+" core_publish="+ISSX_OnOff(core_publish_ok)+" root_projection="+ISSX_OnOff(root_projection_ok)+" internal="+g_last_ea1_stage_write_state+" universe="+g_last_ea1_universe_write_state+" debug="+g_last_ea1_debug_write_state+" payload_json="+g_last_ea1_root_payload_state+" status_json="+g_last_ea1_root_status_state+" root_log="+g_last_ea1_root_debug_state);
   return publish_ok;
  }


bool ISSX_ProjectEA2(const string stage_json,
                     const string debug_snapshot_json)
  {
   ...
   g_debug.Write("INFO","ea2_publish","persist_start",
                 "stage_bytes="+IntegerToString(StringLen(stage_json))+
                 " debug_bytes="+IntegerToString(StringLen(debug_snapshot_json)));
   const bool persist_ok=ISSX_PersistStageJson(issx_stage_ea2,g_ea2.header,g_ea2.manifest,stage_json);
   bool debug_ok=true;
   if(Config.GetBool("project_debug_snapshots"))
      debug_ok=ISSX_UI_Test::ProjectStageSnapshot(g_firm_id,issx_stage_ea2,debug_snapshot_json);

   ISSX_LogStageProjectResult("ea2_publish",persist_ok,debug_ok,true,"ea2_projection");
   return (persist_ok && debug_ok);
  }

bool ISSX_ProjectEA3(const string stage_json,
                     const string debug_snapshot_json)
  {
   ...
   g_debug.Write("INFO","ea3_publish","persist_start",
                 "stage_bytes="+IntegerToString(StringLen(stage_json))+
                 " debug_bytes="+IntegerToString(StringLen(debug_snapshot_json)));
   const bool persist_ok=ISSX_PersistStageJson(issx_stage_ea3,g_ea3.header,g_ea3.manifest,stage_json);
   bool debug_ok=true;
   if(Config.GetBool("project_debug_snapshots"))
      debug_ok=ISSX_UI_Test::ProjectStageSnapshot(g_firm_id,issx_stage_ea3,debug_snapshot_json);

   ISSX_LogStageProjectResult("ea3_publish",persist_ok,debug_ok,true,"ea3_projection");
   return (persist_ok && debug_ok);
  }

bool ISSX_ProjectEA4(const string stage_json,
                     const string debug_snapshot_json)
  {
   ...
   g_debug.Write("INFO","ea4_publish","persist_start",
                 "stage_bytes="+IntegerToString(StringLen(stage_json))+
                 " debug_bytes="+IntegerToString(StringLen(debug_snapshot_json)));
   const bool persist_ok=ISSX_PersistStageJson(issx_stage_ea4,g_ea4.header,g_ea4.manifest,stage_json);
   bool debug_ok=true;
   if(Config.GetBool("project_debug_snapshots"))
      debug_ok=ISSX_UI_Test::ProjectStageSnapshot(g_firm_id,issx_stage_ea4,debug_snapshot_json);

   ISSX_LogStageProjectResult("ea4_publish",persist_ok,debug_ok,true,"ea4_projection");
   return (persist_ok && debug_ok);
  }

bool ISSX_ProjectEA5(const string export_json,
                     const string debug_json)
  {
   ...
   g_debug.Write("INFO","ea5_publish","persist_start",
                 "export_bytes="+IntegerToString(StringLen(export_json))+
                 " debug_bytes="+IntegerToString(StringLen(debug_json))+
                 " root_path="+ISSX_PersistencePath::RootExport(g_firm_id));
   const bool persist_ok=ISSX_PersistStageJson(issx_stage_ea5,g_ea5.header,g_ea5.manifest,export_json);
   const bool root_ok=ISSX_FileIO::WriteTextAtomic(ISSX_PersistencePath::RootExport(g_firm_id),export_json);
   bool debug_ok=true;
   if(Config.GetBool("project_debug_snapshots"))
      debug_ok=ISSX_UI_Test::ProjectStageSnapshot(g_firm_id,issx_stage_ea5,debug_json);

   ISSX_LogStageProjectResult("ea5_publish",persist_ok,debug_ok,root_ok,"ea5_projection");
   return (persist_ok && debug_ok && root_ok);
  }
...
bool ISSX_RunKernelCycle(bool &ea1_stage_ran,string &ea1_stage_result,string &ea1_stage_reason)
  {
   ...
   g_debug.Write((wrapper_snapshot.is_valid_for_analysis?"INFO":"WARN"),"kernel","market_snapshot_state",
   ...
   if(g_ea_enabled[1])
     {
      ...
      if(g_scheduler.RunBatchEx("ea2_history",
                                ...
                                0))
        {
         ...
         const bool ea2_project_ok=ISSX_ProjectEA2(stage_json,debug_json);

                  g_last_ea2_stage_run=((g_ea2.stage_minimum_ready_flag ? (g_ea2.degraded_flag?"degraded":"success") : "blocked"));
                  if(!ea2_project_ok && g_last_ea2_stage_run!="blocked")
                     g_last_ea2_stage_run="degraded";
   ...
   if(g_ea_enabled[2])
     {
      ...
      if(!downstream_analysis_ready)
                                     ...
                                     0))
        {
         ...
         const bool ea3_project_ok=ISSX_ProjectEA3(stage_json,ea3_debug);

         g_last_ea3_stage_run=(g_ea3.stage_minimum_ready_flag ? (g_ea3.degraded_flag?"degraded":"success") : "blocked");
         if(!ea3_project_ok && g_last_ea3_stage_run!="blocked")
            g_last_ea3_stage_run="degraded";
         g_last_ea3_stage_reason=(g_ea3.dependency_block_reason=="" ? (ea3_project_ok?"ok":"projection_failed") : g_ea3.dependency_block_reason);
        }
      else
        {
   ...
   if(g_ea_enabled[3])
     {
      ...
      if(!downstream_analysis_ready)
                                     ...
                                     0))
        {
         ...
         const bool ea4_project_ok=ISSX_ProjectEA4(stage_json,ea4_debug);

         g_last_ea4_stage_run=(g_ea4.stage_minimum_ready_flag ? (g_ea4.degraded_flag?"degraded":"success") : "blocked");
         if(!ea4_project_ok && g_last_ea4_stage_run!="blocked")
            g_last_ea4_stage_run="degraded";
         g_last_ea4_stage_reason=(g_ea4.dependency_block_reason=="" ? (ea4_project_ok?"ok":"projection_failed") : g_ea4.dependency_block_reason);
        }
      else
        {
         ...
         if(ea5_export_due)
           {
            ...
            const bool ea5_project_ok=ISSX_ProjectEA5(export_json,ea5_debug);
            if(!ea5_project_ok)
              {
               g_last_ea5_stage_run="degraded";
               g_last_ea5_stage_reason="projection_failed";
              }
...
bool ISSX_ValidateRatesAvailability(string &reason)
  {
   ...
   snapshot.Reset();
   return ISSX_DataHandler::BuildAndValidateCurrentMarketSnapshot(_Symbol,_Period,snapshot,reason,Config.GetInt("lock_stale_after_sec"));
  }
  ...
  bool ISSX_BuildWrapperSnapshot(ISSX_DataHandler::MarketSnapshot &snapshot,string &reason)
  {
   ...
   if(!ISSX_DataHandler::BuildAndValidateCurrentMarketSnapshot(_Symbol,_Period,snapshot,reason,Config.GetInt("lock_stale_after_sec")))
     {
      g_debug.Write("WARN","readiness","market_snapshot_invalid",
                    ...
                    " stale_after_sec="+IntegerToString(Config.GetInt("lock_stale_after_sec")));
      return false;
     }
...
bool ISSX_ValidateWrapperExecutionReadiness(string &reason,
                                            ISSX_DataHandler::MarketSnapshot &snapshot)
  {
   ...
   if(!ISSX_BuildWrapperSnapshot(snapshot,reason))
     {
      reason=(snapshot.readiness_reason!="" ? snapshot.readiness_reason : reason);
      g_debug.Write("WARN","readiness","wrapper_market_wait",
                    "reason="+reason+
                    " symbol="+_Symbol+
                    " period="+IntegerToString((int)_Period));
      return true;
...
bool ISSX_ValidateStageRegistry(string &reason)
  {
   ...
   if(!g_stage_registry_infra.ValidateRequiredStages(reason))
     {
      g_debug.Write("ERROR","stage_registry","validation_fail","reason="+reason+" summary="+g_stage_registry_infra.BuildSummary());
      g_stage_registry_infra.DumpSummary();
      return false;
     }
...
bool ISSX_FinalReadinessCheck(string &reason)
  {
   ...
   ISSX_DataHandler::MarketSnapshot snapshot;
   snapshot.Reset();
   string snapshot_reason="ok";
   if(!ISSX_DataHandler::BuildAndValidateCurrentMarketSnapshot(_Symbol,_Period,snapshot,snapshot_reason,Config.GetInt("lock_stale_after_sec")))
     {
      reason=(snapshot.readiness_reason!="" ? snapshot.readiness_reason : snapshot_reason);
      g_debug.Write("WARN","startup","market_wait_attach",
                    "reason="+reason+
                    " symbol="+_Symbol+
                    " period="+IntegerToString((int)_Period)+
                    " stale_after_sec="+IntegerToString(Config.GetInt("lock_stale_after_sec")));
...
string ISSX_BuildEA1OperatorStatusJson(const string lifecycle_state,
                                       const string readiness_reason,
                                       const ISSX_DataHandler::MarketSnapshot &snapshot)
  {
   ISSX_JsonWriter j;
   j.Reset();
   j.BeginObject();
   j.NameString("engine_name",ISSX_ENGINE_NAME);
   j.NameString("engine_version",ISSX_ENGINE_VERSION);
   j.NameString("patch_signature",ISSX_PATCH_SIGNATURE);
   j.NameString("build_signature",ISSX_BuildSignature());
   j.NameString("stage_alias",ISSX_StageAlias(issx_stage_ea1));
   j.NameString("lifecycle_state",lifecycle_state);
   j.NameString("readiness_reason",readiness_reason);
   j.NameString("symbol",_Symbol);
   j.NameInt("period",(int)_Period);
   j.NameString("broker",g_operator_broker_name);
   j.NameString("server",g_operator_server_name);
   j.NameLong("login_id",g_operator_login_id);
   j.NameString("boot_id",g_boot_id);
   j.NameBool("runtime_ready",g_runtime_ready);
   j.NameBool("kernel_busy",g_kernel_busy);
   j.NameBool("ea1_enabled_effective",g_ea_enabled[0]);
   j.NameString("ea1_stage_run",g_last_ea1_stage_run);
   j.NameString("ea1_stage_reason",g_last_ea1_stage_reason);
   j.NameString("ea1_publish_state",g_last_ea1_publish_state);
   j.NameString("ea1_publish_reason",g_last_ea1_publish_reason);
   j.NameString("kernel_result",g_last_kernel_result);
   j.NameString("kernel_reason",g_last_kernel_reason);
   j.NameLong("timer_pulse",(long)g_timer_pulse_count);
   j.NameLong("scheduler_cycle_no",(long)g_runtime.State().scheduler_cycle_no);
   j.NameLong("kernel_minute_id",(long)g_runtime.State().kernel.kernel_minute_id);
   j.NameString("stage_registry_ea1",ISSX_StageStateRegistry::StateToString(StageRegistry.GetState("ea1_market")));
   j.NameString("stage_registry_reason",StageRegistry.GetReason("ea1_market"));
   j.NameLong("stage_registry_elapsed_ms",(long)StageRegistry.GetElapsed("ea1_market"));

   j.NameBool("snapshot_present",(snapshot.capture_time>0 || snapshot.last_tick_time>0 || StringLen(snapshot.readiness_reason)>0));
   j.NameBool("market_data_ready",snapshot.is_valid_for_analysis);
   j.NameBool("has_live_tick",snapshot.has_live_tick);
   j.NameBool("has_recent_tick",snapshot.has_recent_tick);
   j.NameBool("has_rates",snapshot.has_rates);
   j.NameBool("history_complete",snapshot.history_complete);
   j.NameBool("symbol_valid",snapshot.symbol_valid);
   j.NameBool("tick_valid",snapshot.tick_valid);
   j.NameBool("prices_valid",snapshot.prices_valid);
   j.NameBool("spec_valid",snapshot.spec_valid);
   j.NameBool("is_stale",snapshot.is_stale);
   j.NameBool("is_cached",snapshot.is_cached);
   j.NameString("snapshot_readiness_reason",snapshot.readiness_reason);
   j.NameString("snapshot_source_reason",snapshot.source_reason);
   j.NameString("snapshot_validation_code",snapshot.validation_code);
   j.NameString("snapshot_cache_state",snapshot.cache_state);
   j.NameLong("snapshot_capture_time",(long)snapshot.capture_time);
   j.NameLong("snapshot_terminal_capture_time",(long)snapshot.terminal_capture_time);
   j.NameLong("last_tick_time",(long)snapshot.last_tick_time);
   j.NameLong("last_bar_time",(long)snapshot.last_bar_time);
   long tick_age_sec=0;
   if(snapshot.capture_time>0 && snapshot.last_tick_time>0)
      tick_age_sec=(long)(snapshot.capture_time-snapshot.last_tick_time);
   j.NameLong("tick_age_sec",tick_age_sec);
   j.NameInt("stale_after_sec",snapshot.tick_snapshot.stale_after_sec);
   j.NameInt("bars_loaded",snapshot.bars_loaded);
   j.NameInt("min_rates_required",snapshot.min_rates_required);
   j.NameDouble("bid",snapshot.bid,Digits());
   j.NameDouble("ask",snapshot.ask,Digits());
   j.NameDouble("spread_points",snapshot.spread_points,1);
   j.EndObject();
   return j.ToString();
  }

bool ISSX_WriteEA1OperatorStatus(const string lifecycle_state,const string readiness_reason)
  {
   if(ISSX_Util::IsEmpty(g_market_status_json_relative_path))
     {
      g_last_ea1_root_status_state="failed";
      return false;
     }
   ...
   snapshot.Reset();
   string snapshot_reason="ok";
   ISSX_DataHandler::BuildAndValidateCurrentMarketSnapshot(_Symbol,_Period,snapshot,snapshot_reason,Config.GetInt("lock_stale_after_sec"));

   const string json=ISSX_BuildEA1OperatorStatusJson(lifecycle_state,readiness_reason,snapshot);
   ISSX_DataHandler::ForensicState fs_market;
   fs_market.Reset();
   const bool ok=ISSX_DataHandler::WritePayloadAtomic(g_market_status_json_relative_path,json,fs_market,true);
   g_last_ea1_root_status_state=(ok?"success":"failed");
   g_last_ea1_publish_state=(snapshot.is_valid_for_analysis?"success":"deferred");
   if(!snapshot.is_valid_for_analysis)
      g_last_ea1_publish_reason=(snapshot.readiness_reason!="" ? snapshot.readiness_reason : readiness_reason);
   g_debug.Write((ok?"INFO":"ERROR"),
                 "ea1_status",
                 (ok?"root_status_write_success":"root_status_write_fail"),
                 "state="+lifecycle_state+
                 " readiness_reason="+readiness_reason+
                 " snapshot_reason="+snapshot_reason+
                 " checkpoint="+fs_market.checkpoint+
                 " final_path="+g_market_status_json_relative_path);
   return ok;
  }

void ISSX_RenderHudCycle()
  {
...
int OnInit()
  {
   ...
   g_debug.Write("INFO","build","signature",ISSX_BuildSignature());
...
const bool req_ui_projection=Config.GetBool("gate_ui_projection_requested");
   ...
   g_debug.Write("INFO","paths","operator_layout",
                 ...
                 " market_payload_path="+g_market_json_relative_path+
                 " market_status_path="+g_market_status_json_relative_path+
   ...
   g_debug.Write("INFO","build","schema","fingerprint="+g_registry.SchemaFingerprintHex()+" patch="+ISSX_PATCH_SIGNATURE);
...
if(!ISSX_ValidateStageRegistry(stage_registry_reason))
  {
   ...
   // 8) final readiness check (framework-fatal only; market wait remains attached)
   ...
   const bool market_wait=(ready_reason!="ok");
   ISSX_WriteEA1OperatorStatus((market_wait?"attached_waiting_market":"attached"),(market_wait?ready_reason:"oninit_ready"));
   ISSX_RenderHudCycle();
   g_debug.Write("INFO","timer","event_set_ok","sec="+IntegerToString(timer_sec));
   g_debug.Write((market_wait?"WARN":"INFO"),"startup",(market_wait?"attached_waiting_market":"readiness_ok"),
                 "symbol="+_Symbol+" period="+IntegerToString(_Period)+" reason="+ready_reason);
   g_debug.Write("INFO","lifecycle","oninit_end","result=INIT_SUCCEEDED reason="+ready_reason);
...
void OnDeinit(const int reason)
  {
   ...
   ISSX_WriteEA1OperatorStatus("detached",g_last_deinit_reason_text);
   g_runtime_ready=false;
   g_bootstrapped=false;
   if(g_menu_initialized)
     {
   ...
   if(!g_runtime_ready)
     {
      ...
      g_last_kernel_reason="runtime_not_ready";
      ISSX_WriteEA1OperatorStatus("waiting","runtime_not_ready");
      if(ISSX_IsUiProjectionOn())
         ISSX_RenderHudCycle();
      return;
     }
   ...
   if(g_kernel_busy)
     {
      ...
      g_last_kernel_result="degraded";
      g_last_kernel_reason="kernel_busy";
      ISSX_WriteEA1OperatorStatus("busy","kernel_busy");
      if(ISSX_IsUiProjectionOn())
         ISSX_RenderHudCycle();
      return;
     }
...
g_memory_guard.Cleanup();
   ...
   if(!ISSX_CanRunScheduler(readiness_reason))
     {
      ...
      g_last_ea1_stage_run="deferred";
      g_last_ea1_stage_reason=readiness_reason;
      g_last_ea1_publish_state="deferred";
      g_last_ea1_publish_reason=readiness_reason;
      ISSX_SyncEA1StageRegistry(g_last_ea1_stage_run,g_last_ea1_stage_reason,0,true);
      ISSX_WriteEA1OperatorStatus("waiting",readiness_reason);
      if(ISSX_IsUiProjectionOn())
         ISSX_RenderHudCycle();
   ...
   if(sampled || !timer_cycle_ok)
      g_debug.Write("INFO","cycle","stage_summary",
                    "ea1="+g_last_ea1_stage_run+"/"+g_last_ea1_stage_reason+
                    " ea1_publish="+g_last_ea1_publish_state+"/"+g_last_ea1_publish_reason+
                    " ea2="+g_last_ea2_stage_run+"/"+g_last_ea2_stage_reason+
                    " ea3="+g_last_ea3_stage_run+"/"+g_last_ea3_stage_reason+
                    " ea4="+g_last_ea4_stage_run+"/"+g_last_ea4_stage_reason+
                    " ea5="+g_last_ea5_stage_run+"/"+g_last_ea5_stage_reason+
                    " kernel="+g_last_kernel_result+"/"+g_last_kernel_reason);
