...
11. deterministic run selection rule in `Aurora Blueprint/runs/`: choose the highest numbered `AURORA_RUN_###.md` file; do not use file mtime
...
## RUN SELECTION RULE (DETERMINISTIC)

When multiple run files share identical modification timestamps, treat the current run as the highest numbered `AURORA_RUN_###.md` value.
Do not use mtime sorting as the control truth selector.
...
- The wrapper canon has completed its owning-pack consolidation refresh and now remains a stable 13-file compiled layer rather than a substitute workspace.
